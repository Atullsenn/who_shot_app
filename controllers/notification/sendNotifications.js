...
const sendNotifications = (req,res)=>{
  ...
  const get_huntadmin_id =(user_id,typeid,selectiontype)=>{
    if(selectiontype === 1){
      if(typeid === 1){
        ...
        var sqldata = "SELECT user_id FROM `tbl_hunt` WHERE id='"+user_id+"' AND end_date IS NULL";
      }else if(typeid === 3){
        //all group user id
        var sqldata = "SELECT a.hunter_id AS user_id FROM tbl_hunters a INNER JOIN tbl_hunt b ON a.hunt_id = b.id WHERE b.end_date IS NULL";
      }else {
        //particular group user id
        var sqldata = "SELECT hunter_id AS user_id FROM tbl_hunters WHERE hunt_id = '"+user_id+"'";
      }
    }else{
      if(typeid === 5){
        var sqldata = "SELECT hunter_id AS user_id FROM `tbl_hunters`";
      }else {
        var sqldata = "SELECT hunter_id AS user_id FROM `tbl_hunters` WHERE hunter_id='"+user_id+"'";
      }
    }
  ...
  const sendNotification = (type,sub_type,user_id,subject,description)=>{
    db.query(`INSERT INTO tbl_notification(type,sub_type,user_id,subject,description) Values('${type}','${sub_type}','${user_id}', '${subject}', '${description}')`,(insertErr, insertData)=>{
      if(insertErr){
        res.status(500).send({
          success: false,
          message: insertErr
