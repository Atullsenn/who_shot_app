...
const getNotificationById = (req,res)=>{
    ...
    const getHuntName = (userId,callback)=>{
        db.query('SELECT a.hunt_name AS name FROM tbl_hunt a INNER JOIN tbl_notification b ON b.user_id = a.id WHERE b.type = 1 AND b.id = "'+userId+'"', (err,data)=>{
            if(err){
                res.status(500).send({
                    success: false,
                    message: err
    ...
    const getHunterName = (userId,callback)=>{
        db.query('SELECT a.full_name AS name FROM tbl_app_users a INNER JOIN tbl_notification b ON a.id = b.user_id WHERE b.type = 2 AND b.id = "'+userId+'"',(err,data)=>{
            if(err){
                res.status(500).send({
                    success: false,
                    message: err
    ...
    db.query('SELECT * FROM tbl_notification WHERE id = "'+notificationID+'"',(err,data)=>{
        if(err){
            res.status(500).send({
                success: false,
                message: err
