...
const getNotificationByHuntId = (req,res)=>{
    ...
    const getHuntName = (user_id, callback)=>{
        db.query('SELECT hunt_name, hunt_image AS profile FROM tbl_hunt WHERE id = "'+user_id+'"',(err,data)=>{
            if(err){
                res.status(500).send({
                    success: false,
                    message: err
    ...
    db.query('SELECT * FROM tbl_notification  WHERE user_id = "'+req.body.user_id+'" AND user_delete = 0',(err,data)=>{
        if(err){
        res.status(500).send({
            success: "false",
            message: err
        ...
        return;
            ...
            db.query('UPDATE tbl_notification SET notification_read = 1 WHERE user_id = "'+req.body.user_id+'"',(err,data)=>{
                if(err){
                  res.status(500).send({
                      success: "false",
                      message: err
