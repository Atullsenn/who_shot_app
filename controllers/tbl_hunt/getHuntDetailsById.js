...
const getHuntDetailsById = (req,res)=>{
    ...
    const getHunterDetails = (huntID, callback)=>{
        db.query('SELECT a.full_name AS hunter, SUM(b.no_of_killed + no_of_wound +  no_of_fire + no_of_missed) AS total, b.no_of_killed , b.no_of_missed, b.no_of_fire, b.no_of_wound FROM tbl_app_users a INNER JOIN tbl_hunters b ON a.id = b.hunter_id WHERE hunt_id = "'+huntID+'"',(err,data)=>{
            if(err){
                res.status(500).send({
                    success: false,
                    message: err
    ...
    //Function
    db.query('SELECT a.*, b.full_name AS admin FROM tbl_hunt a INNER JOIN tbl_app_users b ON a.user_id = b.id WHERE a.id = "'+req.body.id+'"',(err,data)=>{
        if(err){
            res.status(500).send({
                success: false,
                message: err
