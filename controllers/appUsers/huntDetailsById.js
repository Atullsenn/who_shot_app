...
const getHunter = (hunter_id)=>{
    ...
    const hunterData = []
    db.query('SELECT a.id,a.hunter_id, b.full_name, b.profile FROM tbl_hunters a INNER JOIN tbl_app_users b ON a.hunter_id = b.id WHERE hunt_id = "'+hunter_id+'" AND a.hunter_remove_status = 0',(err,data)=>{
        if(err){
            console.log("err")
        }
