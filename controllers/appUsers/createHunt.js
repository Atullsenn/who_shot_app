    ...
    db.query('SELECT hunt_name FROM tbl_hunt WHERE hunt_name = "'+req.body.hunt_name+'"',(err,data)=>{
        ...
        if(data.length > 0){
            ...
            if(req.body.hunt_image == ''){
                db.query(`INSERT INTO tbl_hunt(user_id,group_id,hunt_name,passcode,start_date,start_time,hunt_image) VALUES('${req.body.user_id}','${req.body.group_id}','${req.body.hunt_name}', '${req.body.passcode}', '${req.body.start_date}', '${req.body.start_time}', '')`,(err,data)=>{
                    if(err){
                        res.status(500).send({
                            success: "false",
                            message: err
                ...
                db.query(`INSERT INTO tbl_hunt(user_id,group_id,hunt_name,passcode,start_date,start_time,hunt_image) VALUES('${req.body.user_id}','${req.body.group_id}','${req.body.hunt_name}', '${req.body.passcode}', '${req.body.start_date}', '${req.body.start_time}', '${req.file.filename}')`,(err,data)=>{
                    if(err){
                        res.status(500).send({
                            success: "false",
                            message: err
